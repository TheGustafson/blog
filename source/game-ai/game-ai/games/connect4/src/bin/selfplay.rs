use ai_connect4::{
    Algorithm, GameResult, Move, Position, SEARCH_PRESETS, SearchLimits, Side, iterative_search,
    search_preset,
};
use std::env;
...
fn run() -> Result<(), String> {
    ...
    println!(
        ...
        config.a.name,
        config.a.limits.depth,
        config.a.limits.nodes.unwrap_or_default(),
        config.b.name,
        config.b.limits.depth,
        config.b.limits.nodes.unwrap_or_default(),
        config.games,
    ...
    for game in 0..config.games {
        ...
        let a_is_red = a_has_first_color(game);

        while position.result() == GameResult::Ongoing {
            let a_to_move = (position.side_to_move() == Side::Red) == a_is_red;
            ...
            let report = iterative_search(position, Algorithm::TranspositionTable, player.limits);
        ...
        let outcome = match position.result() {
            GameResult::Win(winner) if (winner == Side::Red) == a_is_red => {
                a_wins += 1;
                "A wins"
            }
            GameResult::Win(_) => {
...
fn parse_config_from(args: Vec<String>) -> Result<Config, String> {
    ...
    if let Some(depth) = a_depth {
        a.limits.depth = depth;
    }
    if let Some(depth) = b_depth {
        b.limits.depth = depth;
    }
    if let Some(nodes) = a_nodes {
        a.limits.nodes = Some(nodes);
    }
    if let Some(nodes) = b_nodes {
        b.limits.nodes = Some(nodes);
    }
    validate(&a)?;
    validate(&b)?;
    Ok(Config {
...
fn validate(player: &Player) -> Result<(), String> {
    if !(1..=42).contains(&player.limits.depth) {
        return Err(format!("{} depth must be from 1 through 42", player.name));
    }
    if !matches!(player.limits.nodes, Some(1..=10_000_000)) {
        return Err(format!(
...
mod tests {
    ...
    fn profiles_and_overrides_are_parsed_without_changing_the_other_player() {
        ...
        assert_eq!(config.a.limits, SearchLimits::with_nodes(8, 12_345));
        assert_eq!(config.b.name, "easy");
        assert_eq!(config.b.limits, SearchLimits::with_nodes(5, 2_000));
    ...
    fn invalid_match_shapes_and_search_limits_are_rejected() {
        ...
        assert!(parse_error(&["--a-depth", "43"]).contains("1 through 42"));
