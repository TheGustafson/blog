...
use gai_chess::{Color, MoveKind, PieceKind, Position};
...
fn main() {
    if let Err(error) = run() {
        eprintln!("gai-chess-psqt-tuner: {error}");
        std::process::exit(2);
    }
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    let path = env::args()
        .nth(1)
        .ok_or("usage: gai-chess-psqt-tuner <FEN|cp|result corpus>")?;
...
fn emit_constants(
    ...
    writeln!(
        output,
        "//! Generated by `gai-chess-psqt-tuner`; do not hand-edit.\n\
         //!\n\
